...
const deleteTodo = (ev)=>{
    ...
    json.map((doc,id)=>{
        if(id!==json.length-1){
            data+=`<div id="todo_items" class=${!doc.done || "done" } ><div style='flex:1' class="done-todo">${doc.Text}</div><img id= 'c${doc.id}' src='checked.png' onclick="checkTodo(this);"  style='width:15px; height:15px; cursor:pointer ' /><img id=${doc.id}  onclick="deleteTodo(this);" src='delete.png' style='width:15px; height:15px; cursor:pointer; margin-left:12px; margin-right:3px ' /></div>`
        }
    })
